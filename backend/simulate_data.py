...
#Imports have to be here in order for the packages to be available to functions imported from this backend.
...
def make_df_parallelized(sorted_filenames_flat, train_multip, valtest_multip, n_processes, xyz_path): #This is the new parallelized one
    ...
    #print(f"It should take about {round(0.2*0.75*(train_multip+2*valtest_multip), 1)} minutes")
    ...
    #Now they're going into dataframes
    ...
    #index_list = np.append(index_list, "biso") #No longer included
    ...
    #Split train and val in X and y
...
def PDF_from_index(xyzfileindex): #Makes a single PDF from an xyz-file with the index as input
    xyz_path = 'xyz_files/natoms200/'    

    #if xyzfileindex == 1000 or xyzfileindex == 2000 or xyzfileindex == 3000 or xyzfileindex == 4000:
    #    print(f"{xyzfileindex} / {len(sorted_filenames_flat)}")
    
    #if xyzfileindex % int(len(sorted_filenames_flat)/10) == 0:            #Shows progress
    #        print(f"{xyzfileindex} / {len(sorted_filenames_flat)}")  #Shows progress
    vary_parameters()
    xgrid, xyz_pdf = PDF_obj.genPDF(xyz_path+sorted_filenames_flat[xyzfileindex], fmt='xyz') #PDF is made here!
    xyz_pdf = xyz_pdf/max(xyz_pdf) #Normalisation
    xyz_pdf = np.append(xyz_pdf, np.array([PDF_obj.qmin, PDF_obj.qmax, PDF_obj.qdamp]))
    xyz_pdf = np.append(xyz_pdf, int(xyzfileindex))
    return xyz_pdf
...
PDF_obj, sorted_filenames_flat, xyz_path = get_simulate_data_backend()
...
def simulate_clusters(folder_name, maximum_atoms, minimum_atoms): #old
...
def simulate_clusters(folder_name, maximum_atoms, minimum_atoms): #tjek
    data_dir = "xyz_files/" + folder_name + "/"
